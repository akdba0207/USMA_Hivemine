    ...
    def publish_messages(self):
        while not rospy.is_shutdown():
            if (self.mode == 5):
                ...
                prev_Northing = self.scale * math.sin(d_t*(self.count-1))
                prev_Easting = self.scale * math.cos(d_t*(self.count-1))

                next_Northing = self.scale * math.sin(d_t*self.count)
                next_Easting = self.scale * math.cos(d_t*self.count)
